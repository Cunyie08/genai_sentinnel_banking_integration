...
import { setRoute, dismissWelcome, triggerPayment } from "../features/uiSlice"; // <--- Added triggerPayment
import { fetchDashboard } from "../features/accountSlice";
import { api } from "../api/axiosConfig";
import {
  Bell, Eye, EyeOff, Copy, QrCode, Send, Plus, Clock, FileText,
  Smartphone, Wifi, Gamepad2, Zap, Sparkles, X,
} from "lucide-react";
...
const HomeScreen = () => {
  ...
  const account = useSelector((state) => state.account?.details); // Safe access
  ...
  useEffect(() => {
    ...
    // C. Check for Pending Web Payments (The "Push to App" feature)
    const checkPayments = async () => {
      try {
        // Wait 2 seconds before showing the alert to feel "real-time"
        setTimeout(async () => {
          const res = await api.checkPaymentRequest();
          if (res.data && res.data.id) {
            dispatch(triggerPayment()); // <--- This triggers the modal!
          }
        }, 2000);
      } catch (err) {
        // No pending payments, ignore
      }
    };
  ...
  const actions = [
    { label: "Send",    icon: Send,     route: "send",    bg: "bg-[#A01030]", color: "text-white" },
    { label: "Fund",    icon: Plus,     route: "fund",    bg: "bg-white",     color: "text-gray-900" },
    { label: "History", icon: Clock,    route: "history", bg: "bg-white",     color: "text-gray-900" },
    { label: "Bills",   icon: FileText, route: "bills",   bg: "bg-white",     color: "text-gray-900" },
  ];
    ...
    <div className="min-h-full w-full bg-[#F8F9FB] font-sans relative">
      ...
      <header className="w-full px-4 sm:px-6 xl:px-8 pt-8 pb-4 flex justify-between items-center bg-[#F8F9FB]/95 backdrop-blur-sm sticky top-0 z-20 border-b border-gray-100">
        <div className="flex items-center gap-3 fu fu1">
          ...
          <div>
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-none mb-0.5">Good Afternoon</p>
            <h2 className="text-base sm:text-lg xl:text-xl font-extrabold text-gray-900 leading-tight">{user?.name || "Tunde"}</h2>
          </div>
        </div>
        <div className="flex gap-2 sm:gap-3 fu fu1">
          <button className="w-9 h-9 sm:w-10 sm:h-10 xl:w-12 xl:h-12 rounded-full bg-white flex items-center justify-center shadow-sm text-[#A01030] hover:bg-rose-50 transition-colors">
            <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 xl:w-6 xl:h-6" fill="currentColor" />
          </button>
        ...
        <div className="fu fu2 w-full bg-gradient-to-br from-[#9F1239] to-[#881337] rounded-[24px] xl:rounded-[32px] p-5 sm:p-6 xl:p-8 text-white shadow-xl shadow-red-900/20 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium uppercase tracking-widest opacity-80">Total Balance</span>
              <button onClick={() => setShowBal(!showBal)} className="opacity-70 hover:opacity-100 transition-opacity focus:outline-none">
            ...
            <div className="bg-[#B45309]/80 backdrop-blur-md px-3 py-1 rounded-full border border-white/10">
              <span className="text-[10px] font-bold text-[#FFD700] uppercase tracking-wider">{account?.tier || 'PREMIUM'}</span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-end relative z-10">
            <div>
              <p className="text-[10px] text-white/60 uppercase font-bold mb-1 tracking-widest">Savings Account</p>
              <div className="flex items-center gap-2">
                <span className="text-sm font-mono font-bold tracking-wider">{account?.number || '0000000000'}</span>
                <Copy size={13} className="opacity-60 cursor-pointer hover:opacity-100 transition-opacity" />
              </div>
            </div>
          ...
          {actions.map((action, i) => (
            <div key={i} className="flex flex-col items-center gap-2 flex-1">
              <button
                ...
                className={`w-14 h-14 sm:w-16 sm:h-16 md:w-18 md:h-18 xl:w-20 xl:h-20 ${action.bg} rounded-[18px] md:rounded-[22px] xl:rounded-[26px] flex items-center justify-center ${action.color} shadow-sm border border-gray-100 active:scale-95 hover:scale-[1.06] transition-transform`}
              >
                <action.icon className="w-6 h-6 md:w-7 md:h-7 xl:w-8 xl:h-8" />
              </button>
              <span className="text-[11px] sm:text-xs md:text-sm font-bold text-gray-700">{action.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="fu fu4">
          ...
          <div className="grid grid-cols-4 gap-3 md:gap-5 xl:gap-8">
            {quickItems.map((item, idx) => (
              <div key={idx} onClick={() => dispatch(setRoute(item.route))} className="flex flex-col items-center gap-2 xl:gap-3 cursor-pointer group">
              ...
              <div className="marquee-track">
                {feedCards?.length > 0 && [...feedCards, ...feedCards].map((card, i) => (
                  ...
                  >
                    ...
                    <button 
                      onClick={() => card.ctaRoute && dispatch(setRoute(card.ctaRoute))}
