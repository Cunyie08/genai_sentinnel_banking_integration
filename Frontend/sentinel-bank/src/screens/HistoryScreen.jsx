...
import { ChevronLeft, Search, ArrowDownLeft, ArrowUpRight, Filter } from 'lucide-react';

const CATEGORY_EMOJI = {
  airtime:  '📱', data:     '🌐', bills:    '⚡',
  transfer: '💸', salary:   '🏦', betting:  '🎮',
  fund:     '💰', default:  '📄',
};
            ...
            <div>
              {filtered.map((tx, i) => (
                <div key={tx.id} className={`flex items-center gap-3 px-4 sm:px-5 py-4 ${i < filtered.length - 1 ? 'border-b border-gray-50' : ''} hover:bg-gray-50/50 transition-colors`}>
                  <span className="text-xl w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center shrink-0 text-base">
                    {CATEGORY_EMOJI[tx.category] || CATEGORY_EMOJI.default}
                  </span>
                  <div className="flex-1 min-w-0">
