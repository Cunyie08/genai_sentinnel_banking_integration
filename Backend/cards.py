...
from Backend.models import Card, Account, User
...
async def get_cards(
    ...
    acc_stmt = select(Account).filter(Account.customer_id == user.customer_id)
    ...
    card_stmt = select(Card).filter(Card.account_id.in_(account_ids))
    card_result = await db.execute(card_stmt)
    return card_result.scalars().all()
...
async def freeze_card(
    ...
    stmt = select(Card).join(Account).filter(
        Card.card_id == cardId,
        Account.customer_id == user.customer_id
    )
    result = await db.execute(stmt)
...
async def set_card_limit(
    ...
    await db.commit()
    await db.refresh(card)
    return {"message": "Card limit updated successfully", "daily_limit": card.daily_limit}
