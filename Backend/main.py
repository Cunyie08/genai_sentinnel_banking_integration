...
_faq_embedding_cache: dict = {"model": None, "embeddings": None, "embeddings_valid": False}
...
async def lifespan(app: FastAPI):
    global orchestrator, _faq_content, _faq_embedding_cache
    ...
    faq_path = (
        ...
        / "app" / "rag" / "knowledge_base" / "faqs" / "FAQ-001.txt"
    )
    if faq_path.exists():
        with open(faq_path, "r", encoding="utf-8") as f:
            _faq_content = f.read()
        logger.info("FAQ content loaded into cache (%d chars).", len(_faq_content))
    else:
        logger.warning("FAQ file not found at startup — FAQ cache is empty.")
 
    # ── Pre-warm FAQ embedding model ───────────────────────────────────────
    # Load the model ONCE at startup so the first /faqs?prompt= request
    # never pays the cold-start penalty, and so the cache is always seeded
    # from a valid FAQ parse before any request can poison it.
    try:
        _warm_faq_embeddings(faq_path)
        logger.info("FAQ embedding cache pre-warmed successfully.")
    except Exception as exc:
        # Non-fatal — /faqs endpoint will retry on first request
        logger.warning("FAQ embedding pre-warm failed: %s", exc)
...
def _warm_faq_embeddings(faq_path: Path) -> None:
    """
    Parse FAQ-001.txt and seed the module-level embedding cache.
 
    Called once during lifespan startup. Idempotent — safe to call again
    if the cache needs to be rebuilt after a file update.
 
    Raises on genuine errors so the caller can log them. Does NOT raise
    if the FAQ file simply has no parseable Q/A pairs (logs a warning).
    """
    faqs = _parse_faqs(faq_path)
    if not faqs:
        logger.warning(
            "_warm_faq_embeddings: FAQ file parsed to 0 Q/A pairs. "
            "Embedding cache NOT updated. Check FAQ-001.txt format."
        )
        return  # Leave cache in its current state — do not poison it
 
    model = SentenceTransformer("all-MiniLM-L6-v2")
    questions  = [f["question"] for f in faqs]
    embeddings = model.encode(questions, convert_to_numpy=True)
 
    _faq_embedding_cache["model"]            = model
    _faq_embedding_cache["embeddings"]       = embeddings
    _faq_embedding_cache["embeddings_valid"] = True
    logger.info("FAQ embeddings cached: %d questions, embedding dim=%d.",
                len(questions), embeddings.shape[1] if embeddings.ndim > 1 else 0)
 
 
def _parse_faqs(faq_path: Path) -> list:
    """
    Parse FAQ-001.txt into a list of {category, question, answer} dicts.
 
    Handles BOTH answer formats present in the codebase:
      Format A (old):         "A: Some answer text"
      Format B (generated):   "[HUMAN_RESPONSE]: Some answer text"
 
    The old parser only handled Format A, so every Q/A pair generated by
    generate_policies.py was silently dropped — producing an empty list
    that poisoned the embedding cache with a (1, 0) array.
 
    Args:
        faq_path: Path to FAQ-001.txt
 
    Returns:
        List of dicts. Empty list if file missing or no parseable pairs.
    """
    ...
    for line in lines:
        ...
        if stripped.startswith("SECTION "):
            # Flush pending Q/A before switching category
            if current_q and current_a:
                ...
                current_a = []
                in_answer = False
            current_category = stripped.split(":", 1)[-1].strip() if ":" in stripped else stripped
            continue
 
        # ── Question line: "Q1: ..." or "Q12: ..." ────────────────────────
        # Uses a regex so "Q" followed by digits and a colon is unambiguous
        # and doesn't match lines like "QUESTION:" or narrative text.
        import re as _re
        if _re.match(r"^Q\d+\s*:", stripped):
            # Flush previous pair
            if current_q and current_a:
                faqs.append({
                    "category": current_category,
        ...
        # ── Answer line — Format B: "[HUMAN_RESPONSE]: ..." ──────────────
        # Generated by BankingPolicyGenerator. The old parser never handled
        # this format, causing every generated FAQ pair to be dropped.
        if stripped.startswith("[HUMAN_RESPONSE]:"):
            in_answer = True
            rest = stripped[len("[HUMAN_RESPONSE]:"):].strip()
            if rest:
                current_a.append(rest)
            continue
 
        # ── Skip technical metadata lines ─────────────────────────────────
        # [TECHNICAL]: and [TRAJECTORY]: lines contain JSON-like tags not
        # meant to be part of the answer shown to customers.
        if stripped.startswith("[TECHNICAL]:") or stripped.startswith("[TRAJECTORY]:"):
            # Do not set in_answer=False — the human response may follow
            continue
 
        # ── Continuation lines (in_answer == True) ────────────────────────
        if in_answer and stripped:
            # Stop collecting if we hit a divider or a new section marker
...
async def get_faqs(prompt: Optional[str] = None):
    ...
    Returns the best matching FAQ if similarity >= threshold, otherwise
    returns a fallback message with contact details.
 
    The embedding model and question embeddings are cached at startup
    (lifespan → _warm_faq_embeddings). The cache guard
    embeddings_valid=False prevents stale/zero-feature arrays from
    reaching sklearn_cosine_sim, which was the source of the
    'shape=(1, 0)' ValueError in production.
    """
    # ── Always resolve the FAQ path the same way ───────────────────────────
    ...
    if not faqs:
        raise HTTPException(
            status_code=503,
            detail=(
                "FAQ data is not available. The FAQ file may be missing or "
                "in an unrecognised format. Contact support@sentinelbank.ng."
            ),
        )
 
    # ── Return all FAQs if no search prompt ───────────────────────────────
    if not prompt:
        return {"faqs": faqs}
 
    # ── Embedding model: use pre-warmed cache or build lazily ──────────────
    # The embeddings_valid flag prevents using a stale cache from a prior
    # failed parse run (the (1, 0) crash scenario).
    cache_valid = (
        _faq_embedding_cache["model"]            is not None
        and _faq_embedding_cache["embeddings"]   is not None
        and _faq_embedding_cache["embeddings_valid"]
        and _faq_embedding_cache["embeddings"].shape[0] == len(faqs)
        # Recompute if FAQ count changed (e.g. after FAQ-001.txt update)
    )
 
    if not cache_valid:
        logger.info(
            "FAQ embedding cache miss or stale — rebuilding for %d FAQs.", len(faqs)
        )
        try:
            _warm_faq_embeddings(faq_path)
        except Exception as exc:
            logger.error("Failed to build FAQ embeddings: %s", exc)
            raise HTTPException(
                status_code=503,
                detail="FAQ search is temporarily unavailable. Please try again shortly.",
            )
 
    model              = _faq_embedding_cache["model"]
    question_embeddings = _faq_embedding_cache["embeddings"]
 
    # ── Safety check before calling sklearn ───────────────────────────────
    # Defensive guard: if embeddings still invalid after rebuild, fail cleanly
    if (question_embeddings is None
            or question_embeddings.ndim < 2
            or question_embeddings.shape[0] == 0
            or question_embeddings.shape[1] == 0):
        logger.error(
            "FAQ embeddings are invalid after rebuild: shape=%s",
            getattr(question_embeddings, "shape", "None"),
        )
        raise HTTPException(
            status_code=503,
            detail="FAQ search unavailable — embedding error. Contact support.",
        )
